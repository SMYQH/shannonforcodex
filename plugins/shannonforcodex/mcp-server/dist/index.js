...
const requiredText = (label) => z.string().refine((value) => value.trim().length > 0, `${label} must not be empty`);
...
function validateBrowserTargets(args) {
...
function validateCapellaSnapshot(type, snapshot) {
...
function resolveWorkspacePath(input) {
    if (isAbsolute(input))
        throw new Error("file_path must be relative to the assessment workspace");
...
function escapeMarkdownField(value) {
...
function readReport(reportPath) {
    ...
    if (parsed.report_meta !== undefined) {
        ...
        if (!meta.success)
            throw new Error("report.json is malformed: report_meta is invalid");
    }
    const findings = z.array(reportFindingSchema).safeParse(parsed.findings);
    if (!findings.success)
        throw new Error("report.json is malformed: findings are invalid");
    return parsed;
}
function renderNarrative(value) {
    ...
    return value.flatMap((item) => {
        if (item.kind === "prose")
            return [escapeMarkdownText(item.text), ""];
...
function renderReport(report) {
    ...
    if (meta) {
        lines.push("## Assessment", "", `- Target: ${escapeMarkdownField(meta.target)}`, `- Assessment date: ${escapeMarkdownField(meta.assessment_date)}`, `- Scope: ${escapeMarkdownField(meta.scope)}`, "", escapeMarkdownText(meta.executive_summary), "");
    }
    lines.push("## Findings", "");
    if (report.findings.length === 0) {
        ...
        for (const finding of report.findings) {
            lines.push(`### ${escapeMarkdownField(finding.finding_id)}: ${escapeMarkdownField(finding.title)}`, "", `**Severity:** ${escapeMarkdownField(finding.severity)}`, "", escapeMarkdownText(finding.overview), "");
            for (const [label, value] of [
                ...
                if (value)
                    lines.push(`**${label}:** ${escapeMarkdownField(value)}`, "");
            }
            ...
            if (finding.proof_of_impact)
                lines.push("**Proof of impact:**", "", ...renderNarrative(finding.proof_of_impact));
            if (finding.notes)
                lines.push("**Notes:**", "", ...renderNarrative(finding.notes));
...
server.registerTool("save_deliverable", {
    ...
    description: "Save the complete phase deliverable under its canonical filename. Supports analysis, exploitation evidence Markdown, and Capella JSON snapshots. Replaces the previous snapshot of this phase.",
    inputSchema: {
    ...
    if (content === undefined && file_path === undefined)
        throw new Error("Provide content or file_path");
    if (content !== undefined && file_path !== undefined)
        throw new Error("Provide only one of content or file_path");
    if (file_path !== undefined && file_path.trim() === "")
        throw new Error("file_path must be a non-empty workspace-relative path");
    const text = content ?? readFileSync(resolveWorkspacePath(file_path), "utf-8");
...
server.registerTool("set_report_meta", {
    ...
    if (current.report_meta && (current.report_meta.target !== target || current.report_meta.assessment_date !== assessment_date)) {
...
server.registerTool("submit_exploitation_queue", {
    title: "Submit exploitation queue",
    description: "Merge findings into a vuln-class queue by stable ID. Distinct analysis and SAST IDs are retained; identical retries are accepted and conflicting IDs are rejected. An empty submission preserves the queue.",
    inputSchema: {
    ...
    for (const vulnerability of vulnerabilities) {
        if (ids.has(vulnerability.ID))
            throw new Error(`Duplicate queue ID: ${vulnerability.ID}`);
...
server.registerTool("add_finding", {
    ...
    description: "Record a single report finding as structured data. Call once per finding after set_report_meta. Rejects duplicate finding IDs.",
    ...
    if (!current.report_meta)
        throw new Error("Call set_report_meta before add_finding");
...
server.registerTool("submit_task_groups", {
    title: "Submit task groups",
    description: "Record the deduplication groups produced by a task-formation skill. Each label may appear in only one group and must exist in the matching exploitation queue.",
    inputSchema: {
        vuln_class: queueVulnClassSchema,
    ...
    for (const group of groups) {
        for (const label of group.queue_labels) {
            if (labels.has(label))
                throw new Error(`Queue label appears in multiple groups: ${label}`);
            labels.add(label);
        }
    }
    const dir = deliverablesDir();
    ...
    if (existsSync(queuePath)) {
        ...
        for (const label of labels) {
            if (!known.has(label))
                throw new Error(`Queue label not found in ${vuln_class} queue: ${label}`);
        }
    }
...
server.registerTool("playwright_cli", {
    title: "Run playwright-cli",
    description: "Drive a real browser via the trusted plugin-local playwright-cli binary for recon, exploitation proof, and login validation. SHANNON_ALLOWED_ORIGINS is required; always pass an isolated session (-s=<session>).",
    inputSchema: {
        args: z
            .array(z.string())
